...
public class FormatUnitsAndRoots extends Format {
  ...
  protected boolean isRoot(long i) {
    return rh.toRange(i, n) <= sqrtN;
  }
  
  protected double targetWeight(long i) {
    return isRoot(i) ? sqrtN : 1;
  }
  ...
  public Expectation expected(double p, long i) {
    ...
    return new Expectation(w, Math.pow(Math.abs(w), p) / (n + sqrtN * (Math.pow(sqrtN, p) - 1)));
  }
}